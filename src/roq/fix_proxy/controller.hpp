...
struct Controller final : public io::sys::Signal::Handler,
 ...
 protected:
  ...
  // - quotes
  void operator()(Trace<fix::codec::MassQuoteAck> const &) override;
  void operator()(Trace<fix::codec::QuoteStatusReport> const &) override;

  // client::Session::Handler
  void operator()(Trace<client::Session::Disconnected> const &, uint64_t session_id) override;
  // - user
  void operator()(Trace<fix::codec::UserRequest> const &, uint64_t session_id) override;
  // - security
  void operator()(Trace<fix::codec::SecurityListRequest> const &, uint64_t session_id) override;
  void operator()(Trace<fix::codec::SecurityDefinitionRequest> const &, uint64_t session_id) override;
  void operator()(Trace<fix::codec::SecurityStatusRequest> const &, uint64_t session_id) override;
  // - market data
  void operator()(Trace<fix::codec::MarketDataRequest> const &, uint64_t session_id) override;
  // - orders
  void operator()(Trace<fix::codec::OrderStatusRequest> const &, uint64_t session_id) override;
  void operator()(Trace<fix::codec::NewOrderSingle> const &, uint64_t session_id) override;
  void operator()(Trace<fix::codec::OrderCancelReplaceRequest> const &, uint64_t session_id) override;
  void operator()(Trace<fix::codec::OrderCancelRequest> const &, uint64_t session_id) override;
  void operator()(Trace<fix::codec::OrderMassStatusRequest> const &, uint64_t session_id) override;
  void operator()(Trace<fix::codec::OrderMassCancelRequest> const &, uint64_t session_id) override;
  // - positions
  void operator()(Trace<fix::codec::RequestForPositions> const &, uint64_t session_id) override;
  // - trades
  void operator()(Trace<fix::codec::TradeCaptureReportRequest> const &, uint64_t session_id) override;
  // - quotes
  void operator()(Trace<fix::codec::MassQuote> const &, uint64_t session_id) override;
  void operator()(Trace<fix::codec::QuoteCancel> const &, uint64_t session_id) override;
  ...
  void ensure_cl_ord_id(std::string_view const &cl_ord_id, fix::OrdStatus);
 ...
 private:
  ...
  struct {
    ...
    utils::unordered_map<std::string, fix::OrdStatus> state;
  } cl_ord_id_;
  // WORK-AROUND
