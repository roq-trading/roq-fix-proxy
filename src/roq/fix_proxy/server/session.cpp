...
void Session::parse(Trace<fix::Message> const &event) {
  ...
  switch (header.msg_type) {
    ...
    case TRADE_CAPTURE_REPORT: {
      ...
      // quotes
    case MASS_QUOTE_ACK: {
      auto mass_quote_ack = fix::codec::MassQuoteAck::create(message, decode_buffer_, decode_buffer_2_);
      dispatch(event, mass_quote_ack);
      break;
    }
    case QUOTE_STATUS_REPORT: {
      auto quote_status_report = fix::codec::QuoteStatusReport::create(message);
...
void Session::operator()(Trace<fix::codec::ResendRequest> const &event, fix::Header const &) {
  auto &[trace_info, resend_request] = event;
  log::debug("resend_request={}, trace_info={}"sv, resend_request, trace_info);
}

void Session::operator()(Trace<fix::codec::Logon> const &event, fix::Header const &) {
...
void Session::operator()(Trace<fix::codec::Logout> const &event, fix::Header const &) {
...
void Session::operator()(Trace<fix::codec::Heartbeat> const &event, fix::Header const &) {
  auto &[trace_info, heartbeat] = event;
  log::debug("heartbeat={}, trace_info={}"sv, heartbeat, trace_info);
}

void Session::operator()(Trace<fix::codec::TestRequest> const &event, fix::Header const &) {
  auto &[trace_info, test_request] = event;
  send_heartbeat(test_request.test_req_id);
}

void Session::operator()(Trace<fix::codec::BusinessMessageReject> const &event, fix::Header const &) {
...
void Session::operator()(Trace<fix::codec::SecurityList> const &event, fix::Header const &) {
  auto &[trace_info, security_list] = event;
  log::debug("security_list={}, trace_info={}"sv, security_list, trace_info);
  handler_(event);
}

void Session::operator()(Trace<fix::codec::SecurityDefinition> const &event, fix::Header const &) {
  auto &[trace_info, security_definition] = event;
  log::debug("security_definition={}, trace_info={}"sv, security_definition, trace_info);
  handler_(event);
}

void Session::operator()(Trace<fix::codec::SecurityStatus> const &event, fix::Header const &) {
  auto &[trace_info, security_status] = event;
  log::debug("security_status={}, trace_info={}"sv, security_status, trace_info);
  handler_(event);
}

void Session::operator()(Trace<fix::codec::MarketDataRequestReject> const &event, fix::Header const &) {
...
void Session::operator()(Trace<fix::codec::MarketDataSnapshotFullRefresh> const &event, fix::Header const &) {
...
void Session::operator()(Trace<fix::codec::MarketDataIncrementalRefresh> const &event, fix::Header const &) {
...
void Session::operator()(Trace<fix::codec::UserResponse> const &event, fix::Header const &) {
  auto &[trace_info, user_response] = event;
  log::debug("user_response={}, trace_info={}"sv, user_response, trace_info);
  handler_(event);
}

void Session::operator()(Trace<fix::codec::OrderCancelReject> const &event, fix::Header const &) {
  auto &[trace_info, order_cancel_reject] = event;
  log::debug("order_cancel_reject={}, trace_info={}"sv, order_cancel_reject, trace_info);
  handler_(event);
}

void Session::operator()(Trace<fix::codec::OrderMassCancelReport> const &event, fix::Header const &) {
...
void Session::operator()(Trace<fix::codec::ExecutionReport> const &event, fix::Header const &) {
  auto &[trace_info, execution_report] = event;
  log::debug("execution_report={}, trace_info={}"sv, execution_report, trace_info);
  handler_(event);
}

void Session::operator()(Trace<fix::codec::RequestForPositionsAck> const &event, fix::Header const &) {
...
void Session::operator()(Trace<fix::codec::PositionReport> const &event, fix::Header const &) {
  auto &[trace_info, position_report] = event;
  log::debug("position_report={}, trace_info={}"sv, position_report, trace_info);
  handler_(event);
}

void Session::operator()(Trace<fix::codec::TradeCaptureReportRequestAck> const &event, fix::Header const &) {
...
void Session::operator()(Trace<fix::codec::TradeCaptureReport> const &event, fix::Header const &) {
  auto &[trace_info, trade_capture_report] = event;
  log::debug("trade_capture_report={}, trace_info={}"sv, trade_capture_report, trace_info);
  handler_(event);
}

void Session::operator()(Trace<fix::codec::MassQuoteAck> const &event, fix::Header const &) {
  auto &[trace_info, mass_quote_ack] = event;
  log::debug("mass_quote_ack={}, trace_info={}"sv, mass_quote_ack, trace_info);
  handler_(event);
}

void Session::operator()(Trace<fix::codec::QuoteStatusReport> const &event, fix::Header const &) {
...
void Session::send_logon() {
  auto heart_bt_int = static_cast<decltype(fix::codec::Logon::heart_bt_int)>(std::chrono::duration_cast<std::chrono::seconds>(ping_freq_).count());
  auto logon = fix::codec::Logon{
