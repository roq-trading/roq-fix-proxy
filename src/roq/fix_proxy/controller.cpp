...
auto get_client_from_parties(T &value) -> std::string_view {
  ...
  auto const &party_ids = [&]() {
    if constexpr (std::is_same<value_type, fix::codec::TradeCaptureReport>::value) {
  ...
  if (std::size(party_ids) == 1)
    for (auto &item : party_ids) {
      if (!std::empty(item.party_id) && item.party_id_source == fix::PartyIDSource::PROPRIETARY_CUSTOM_CODE && item.party_role == fix::PartyRole::CLIENT_ID)
...
auto is_order_complete(auto ord_status) {
  return utils::is_order_complete(map(ord_status));
}

auto is_pending(auto exec_type) {
  if (exec_type == fix::ExecType::PENDING_NEW || exec_type == fix::ExecType::PENDING_REPLACE || exec_type == fix::ExecType::PENDING_CANCEL)
    return true;
  return false;
}
...
auto get_subscription_request_type(auto &event) {
  ...
  if (result == fix::SubscriptionRequestType::UNDEFINED)
    result = fix::SubscriptionRequestType::SNAPSHOT;
  return result;
}
}  // namespace
...
void Controller::operator()(Trace<fix::codec::UserResponse> const &event) {
...
void Controller::operator()(Trace<fix::codec::SecurityList> const &event) {
  auto remove = true;
  auto dispatch = [&](auto session_id, auto &req_id, auto keep_alive) {
    auto failure = event.value.security_request_result != fix::SecurityRequestResult::VALID;
...
void Controller::operator()(Trace<fix::codec::SecurityDefinition> const &event) {
  auto remove = true;
  auto dispatch = [&](auto session_id, auto &req_id, auto keep_alive) {
    auto failure = event.value.security_response_type != fix::SecurityResponseType::ACCEPT_SECURITY_PROPOSAL_AS_IS;
    remove = failure || !keep_alive;
...
void Controller::operator()(Trace<fix::codec::SecurityStatus> const &event) {
  auto remove = true;
  auto dispatch = [&](auto session_id, auto &req_id, auto keep_alive) {
...
void Controller::operator()(Trace<fix::codec::MarketDataRequestReject> const &event) {
...
void Controller::operator()(Trace<fix::codec::MarketDataSnapshotFullRefresh> const &event) {
  auto remove = true;
  auto dispatch = [&](auto session_id, auto &req_id, auto keep_alive) {
    remove = !keep_alive;
...
void Controller::operator()(Trace<fix::codec::MarketDataIncrementalRefresh> const &event) {
  auto dispatch = [&](auto session_id, auto &req_id, [[maybe_unused]] auto keep_alive) {
...
void Controller::operator()(Trace<fix::codec::OrderCancelReject> const &event) {
  auto dispatch = [&](auto session_id, auto &req_id, [[maybe_unused]] auto keep_alive) {
...
void Controller::operator()(Trace<fix::codec::OrderMassCancelReport> const &event) {
  auto dispatch = [&](auto session_id, auto &req_id, [[maybe_unused]] auto keep_alive) {
...
void Controller::operator()(Trace<fix::codec::ExecutionReport> const &event) {
  ...
  if (has_ord_status_req_id) {                                 // order status request
    if (execution_report.ord_status == fix::OrdStatus::REJECTED) {
      // note! no order
    } else {
    ...
    if (execution_report.exec_type == fix::ExecType::REJECTED) {
...
void Controller::operator()(Trace<fix::codec::RequestForPositionsAck> const &event) {
  auto remove = true;
  auto dispatch = [&](auto session_id, auto &req_id, [[maybe_unused]] auto keep_alive) {
    auto failure = event.value.pos_req_result != fix::PosReqResult::VALID || event.value.pos_req_status == fix::PosReqStatus::REJECTED;
    if (failure) {
      remove = true;
...
void Controller::operator()(Trace<fix::codec::PositionReport> const &event) {
  ...
  auto remove = false;
  auto dispatch = [&](auto session_id, auto &req_id, auto keep_alive) {
    auto failure = event.value.pos_req_result != fix::PosReqResult::VALID;
    if (failure) {
      remove = true;
...
void Controller::operator()(Trace<fix::codec::TradeCaptureReportRequestAck> const &event) {
  auto remove = true;
  auto dispatch = [&](auto session_id, auto &req_id, auto keep_alive) {
    remove = !keep_alive;
...
void Controller::operator()(Trace<fix::codec::TradeCaptureReport> const &event) {
  auto remove = true;
  auto dispatch = [&](auto session_id, auto &req_id, auto keep_alive) {
...
void Controller::operator()(Trace<fix::codec::MassQuoteAck> const &) {
  // XXX FIXME TODO
}

void Controller::operator()(Trace<fix::codec::QuoteStatusReport> const &) {
  // XXX FIXME TODO
}
...
void Controller::operator()(Trace<client::Session::Disconnected> const &event, uint64_t session_id) {
  auto unsubscribe_market_data = [&](auto &req_id) {
    if (!ready())
      return;
    auto market_data_request = fix::codec::MarketDataRequest{
        .md_req_id = req_id,
        .subscription_request_type = fix::SubscriptionRequestType::UNSUBSCRIBE,
        .market_depth = {},
        .md_update_type = {},
        .aggregated_book = {},
  ...
  if (iter_2 != std::end(subscriptions_.user.session_to_client)) {
    ...
    if (ready()) {
      ...
      auto user_request = fix::codec::UserRequest{
          .user_request_id = user_request_id,
          .user_request_type = fix::UserRequestType::LOG_OFF_USER,
          .username = username_2,
          .password = {},
          .new_password = {},
      };
...
void Controller::operator()(Trace<fix::codec::UserRequest> const &event, uint64_t session_id) {
...
void Controller::operator()(Trace<fix::codec::SecurityListRequest> const &event, uint64_t session_id) {
  ...
  auto reject = [&]() {
    ...
    auto security_list = fix::codec::SecurityList{
        .security_req_id = req_id,
        .security_response_id = request_id,
        .security_request_result = fix::SecurityRequestResult::INVALID_OR_UNSUPPORTED,
        .no_related_sym = {},
    };
    Trace event_2{event.trace_info, security_list};
    dispatch_to_client(event_2, session_id);
  ...
  auto dispatch = [&](auto keep_alive) {
    ...
    if (exists) {
      assert(subscription_request_type == fix::SubscriptionRequestType::UNSUBSCRIBE);
      remove_req_id(mapping, request_id);  // note! protocol doesn't have an ack for unsubscribe
    } else {
      assert(
          subscription_request_type == fix::SubscriptionRequestType::SNAPSHOT || subscription_request_type == fix::SubscriptionRequestType::SNAPSHOT_UPDATES);
  ...
  switch (subscription_request_type) {
    using enum fix::SubscriptionRequestType;
    case UNDEFINED:
    case UNKNOWN:
      reject();
      break;
...
void Controller::operator()(Trace<fix::codec::SecurityDefinitionRequest> const &event, uint64_t session_id) {
  auto &security_definition_request = event.value;
  auto req_id = security_definition_request.security_req_id;
  auto reject = [&]() {
    auto request_id = shared_.create_request_id();
    auto security_definition = fix::codec::SecurityDefinition{
        .security_req_id = security_definition_request.security_req_id,
        .security_response_id = request_id,
        .security_response_type = fix::SecurityResponseType::REJECT_SECURITY_PROPOSAL,
...
void Controller::operator()(Trace<fix::codec::SecurityStatusRequest> const &event, uint64_t session_id) {
  ...
  auto reject = [&]() {
    ...
    auto security_status = fix::codec::SecurityStatus{
...
void Controller::operator()(Trace<fix::codec::MarketDataRequest> const &event, uint64_t session_id) {
  ...
  auto reject = [&](auto md_req_rej_reason, auto error) {
    auto market_data_request_reject = fix::codec::MarketDataRequestReject{
        .md_req_id = req_id,
  ...
  if (!market_data_request.is_valid()) {
    reject(
        fix::MDReqRejReason::UNSUPPORTED_SCOPE,  // XXX FIXME what to use ???
        fix::codec::Error::VALIDATION);
    return;
  }
  auto &mapping = subscriptions_.md_req_id;
  auto &client_to_server = mapping.client_to_server[session_id];
  ...
  auto dispatch = [&](auto keep_alive) {
    ...
    // note! *after* request has been sent
    if (exists) {
      assert(market_data_request.subscription_request_type == fix::SubscriptionRequestType::UNSUBSCRIBE);
      remove_req_id(mapping, request_id);  // note! protocol doesn't have an ack for unsubscribe
    } else {
      assert(
          market_data_request.subscription_request_type == fix::SubscriptionRequestType::SNAPSHOT ||
          market_data_request.subscription_request_type == fix::SubscriptionRequestType::SNAPSHOT_UPDATES);
      client_to_server.emplace(req_id, request_id);
      mapping.server_to_client.try_emplace(request_id, session_id, req_id, keep_alive);
    }
  };
  switch (market_data_request.subscription_request_type) {
    ...
    case UNDEFINED:
    case UNKNOWN:
      reject(fix::MDReqRejReason::UNSUPPORTED_SUBSCRIPTION_REQUEST_TYPE, fix::codec::Error::UNKNOWN_SUBSCRIPTION_REQUEST_TYPE);
      break;
    case SNAPSHOT:
      if (exists) {
        reject(fix::MDReqRejReason::DUPLICATE_MD_REQ_ID, fix::codec::Error::DUPLICATE_MD_REQ_ID);
      } else {
        dispatch(false);
      }
      break;
    ...
    case UNSUBSCRIBE:
      if (exists) {
        ...
        reject(
            fix::MDReqRejReason::UNSUPPORTED_SUBSCRIPTION_REQUEST_TYPE,  // XXX FIXME what to use ???
            fix::codec::Error::UNKNOWN_MD_REQ_ID);
      }
      break;
  }
}

void Controller::operator()(Trace<fix::codec::OrderStatusRequest> const &event, uint64_t session_id) {
  auto &order_status_request = event.value;
  auto reject = [&](auto ord_rej_reason, auto error) {
    auto request_id = shared_.create_request_id();
    auto execution_report = fix::codec::ExecutionReport{
        ...
        .exec_type = fix::ExecType::ORDER_STATUS,  // required
        .ord_status = fix::OrdStatus::REJECTED,    // required
        .working_indicator = {},
  ...
  if (!order_status_request.is_valid()) {
    reject(fix::OrdRejReason::OTHER, fix::codec::Error::VALIDATION);
    return;
  }
  ...
  if (!std::empty(req_id)) {  // note! optional
    ...
    if (iter != std::end(client_to_server)) {
      reject(fix::OrdRejReason::OTHER, fix::codec::Error::DUPLICATE_ORD_STATUS_REQ_ID);
      return;
    }
  }
...
void Controller::operator()(Trace<fix::codec::NewOrderSingle> const &event, uint64_t session_id) {
  ...
  auto iter = client_to_server.find(req_id);
  if (iter != std::end(client_to_server)) {
    reject(fix::OrdRejReason::OTHER, fix::codec::Error::DUPLICATE_CL_ORD_ID);
    return;
  }
...
void Controller::operator()(Trace<fix::codec::OrderCancelReplaceRequest> const &event, uint64_t session_id) {
  ...
  auto reject = [&](auto &order_id, auto ord_status, auto cxl_rej_reason, auto error) {
    ...
    auto order_cancel_reject = fix::codec::OrderCancelReject{
        ...
        .cxl_rej_response_to = fix::CxlRejResponseTo::ORDER_CANCEL_REPLACE_REQUEST,  // required
  ...
  if (!order_cancel_replace_request.is_valid()) {
    reject(ORDER_ID_NONE, fix::OrdStatus::REJECTED, fix::CxlRejReason::OTHER, fix::codec::Error::VALIDATION);
    return;
  }
  ...
  if (iter != std::end(client_to_server)) {
    reject(
        ORDER_ID_NONE,
        fix::OrdStatus::REJECTED,  // XXX FIXME should be latest "known"
        fix::CxlRejReason::DUPLICATE_CL_ORD_ID,
        fix::codec::Error::DUPLICATE_CL_ORD_ID);
    return;
  }
  auto request_id = create_request_id(client_id, req_id);
...
void Controller::operator()(Trace<fix::codec::OrderCancelRequest> const &event, uint64_t session_id) {
  ...
  auto reject = [&](auto &order_id, auto ord_status, auto cxl_rej_reason, auto error) {
    ...
    auto order_cancel_reject = fix::codec::OrderCancelReject{
        ...
        .cxl_rej_response_to = fix::CxlRejResponseTo::ORDER_CANCEL_REQUEST,  // required
        .cxl_rej_reason = cxl_rej_reason,
        .text = magic_enum::enum_name(error),
    };
    Trace event_2{event.trace_info, order_cancel_reject};
  ...
  if (iter != std::end(client_to_server)) {
    reject(
        ...
        fix::OrdStatus::REJECTED,  // XXX FIXME should be latest "known"
        fix::CxlRejReason::DUPLICATE_CL_ORD_ID,
        fix::codec::Error::DUPLICATE_ORD_STATUS_REQ_ID);
    return;
  }
...
void Controller::operator()(Trace<fix::codec::OrderMassStatusRequest> const &event, uint64_t session_id) {
  ...
  auto iter = client_to_server.find(req_id);
  if (iter != std::end(client_to_server)) {
    reject(fix::OrdRejReason::OTHER, fix::codec::Error::DUPLICATE_MASS_STATUS_REQ_ID);
    return;
  }
...
void Controller::operator()(Trace<fix::codec::OrderMassCancelRequest> const &event, uint64_t session_id) {
  auto &order_mass_cancel_request = event.value;
  auto reject = [&](auto order_mass_reject_reason, auto error) {
    auto order_mass_cancel_report = fix::codec::OrderMassCancelReport{
        ...
        .mass_cancel_response = fix::MassCancelResponse::CANCEL_REQUEST_REJECTED,        // required
  ...
  if (!order_mass_cancel_request.is_valid()) {
    reject(fix::MassCancelRejectReason::OTHER, fix::codec::Error::VALIDATION);
    return;
  }
  ...
  auto iter = client_to_server.find(req_id);
  if (iter != std::end(client_to_server)) {
    reject(fix::MassCancelRejectReason::OTHER, fix::codec::Error::DUPLICATE_CL_ORD_ID);
    return;
  }
...
void Controller::operator()(Trace<fix::codec::RequestForPositions> const &event, uint64_t session_id) {
  auto &request_for_positions = event.value;
  auto req_id = request_for_positions.pos_req_id;
  auto reject = [&](auto error) {
    auto request_id = shared_.create_request_id();
    auto request_for_positions_ack = fix::codec::RequestForPositionsAck{
        .pos_maint_rpt_id = request_id,  // required
        .pos_req_id = req_id,
        .total_num_pos_reports = {},
        .unsolicited_indicator = false,
        .pos_req_result = fix::PosReqResult::INVALID_OR_UNSUPPORTED,  // required
        .pos_req_status = fix::PosReqStatus::REJECTED,                // required
  ...
  if (!request_for_positions.is_valid()) {
    reject(fix::codec::Error::VALIDATION);
    return;
  }
  auto &mapping = subscriptions_.pos_req_id;
  auto &client_to_server = mapping.client_to_server[session_id];
  ...
  auto dispatch = [&](auto keep_alive) {
    ...
    // note! *after* request has been sent
    if (exists) {
      assert(subscription_request_type == fix::SubscriptionRequestType::UNSUBSCRIBE);  // see below
      ...
      assert(
          subscription_request_type == fix::SubscriptionRequestType::SNAPSHOT ||
          subscription_request_type == fix::SubscriptionRequestType::SNAPSHOT_UPDATES);  // see below
      client_to_server.emplace(req_id, request_id);
      mapping.server_to_client.try_emplace(request_id, session_id, req_id, keep_alive);
    }
  };
  switch (subscription_request_type) {
    ...
    case UNDEFINED:
    case UNKNOWN:
      reject(fix::codec::Error::UNKNOWN_SUBSCRIPTION_REQUEST_TYPE);
      break;
    case SNAPSHOT:
      if (exists) {
        reject(fix::codec::Error::DUPLICATED_POS_REQ_ID);
      } else {
        dispatch(false);
      }
      break;
    ...
    case UNSUBSCRIBE:
      if (exists) {
        ...
        reject(fix::codec::Error::UNKNOWN_POS_REQ_ID);
      }
      break;
  }
}

void Controller::operator()(Trace<fix::codec::TradeCaptureReportRequest> const &event, uint64_t session_id) {
  auto &trade_capture_report_request = event.value;
  auto req_id = trade_capture_report_request.trade_request_id;
  auto reject = [&](auto error) {
    auto request_id = shared_.create_request_id();
    auto trade_capture_report_request_ack = fix::codec::TradeCaptureReportRequestAck{
        .trade_request_id = req_id,                                             // required
        .trade_request_type = trade_capture_report_request.trade_request_type,  // required
        .trade_request_result = fix::TradeRequestResult::OTHER,                 // required
        .trade_request_status = fix::TradeRequestStatus::REJECTED,              // required
  ...
  switch (subscription_request_type) {
    ...
    case SNAPSHOT:
      if (exists) {
        reject(fix::codec::Error::DUPLICATE_TRADE_REQUEST_ID);
      } else {
        dispatch(false);
      }
      break;
    ...
    case UNSUBSCRIBE:
      if (exists) {
        ...
        reject(fix::codec::Error::UNKNOWN_TRADE_REQUEST_ID);
      }
      break;
  }
}

void Controller::operator()(Trace<fix::codec::MassQuote> const &, [[maybe_unused]] uint64_t session_id) {
  // XXX FIXME TODO
}

void Controller::operator()(Trace<fix::codec::QuoteCancel> const &, [[maybe_unused]] uint64_t session_id) {
  // XXX FIXME TODO
}
...
void Controller::ensure_cl_ord_id(std::string_view const &cl_ord_id, fix::OrdStatus ord_status) {
  if (std::empty(cl_ord_id))
    return;
