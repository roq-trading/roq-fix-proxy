  ...
  switch (state_) {
    ...
    case WAITING_CREATE_ROUTE: {
      ...
      if (user_response_timeout_ < event.value.now) {
        auto const error = fix::codec::Error::USER_RESPONSE_TIMEOUT;
    ...
    case READY:
      if ((last_update_ + shared_.settings.client.heartbeat_freq) < event.value.now) {
        ...
        if (waiting_for_heartbeat_) {
          ...
          auto const error = fix::codec::Error::MISSING_HEARTBEAT;
          auto logout = fix::codec::Logout{
              .text = magic_enum::enum_name(error),
          };
          send_and_close<2>(logout);
    ...
    case WAITING_CREATE_ROUTE:
      switch (user_response.user_status) {
        ...
        case LOGGED_IN: {
          ...
          auto response = fix::codec::Logon{
              .encrypt_method = fix::EncryptMethod::NONE,
      ...
      switch (user_response.user_status) {
        ...
        case NOT_LOGGED_IN: {
          auto success = [&]() {
            // username_.clear();
            // party_id_.clear();
            auto const error = fix::codec::Error::GOODBYE;
            auto response = fix::codec::Logout{
                .text = magic_enum::enum_name(error),
            };
...
#ifndef NDEBUG
  auto can_send = [&]() {
    if constexpr (std::is_same<T, fix::codec::Logon>::value) {
  ...
  switch (message.header.msg_type) {
    ...
    default:
      ...
      send_business_message_reject(
          ...
          fix::BusinessRejectReason::UNSUPPORTED_MESSAGE_TYPE,
          fix::codec::Error::UNEXPECTED_MSG_TYPE);
      break;
  };
}
...
void Session::operator()(Trace<fix::codec::TestRequest> const &event, fix::Header const &header) {
  ...
  switch (state_) {
    using enum State;
    case WAITING_LOGON:
      send_reject_and_close(header, fix::SessionRejectReason::OTHER, fix::codec::Error::NO_LOGON);
      break;
    case WAITING_CREATE_ROUTE:
    case READY: {
...
void Session::operator()(Trace<fix::codec::ResendRequest> const &event, fix::Header const &header) {
  ...
  switch (state_) {
    ...
    case WAITING_CREATE_ROUTE:
      ...
      break;
    case READY:
      send_reject_and_close(header, fix::SessionRejectReason::OTHER, fix::codec::Error::UNSUPPORTED_MSG_TYPE);
      break;
    case WAITING_REMOVE_ROUTE:
      make_zombie();
      break;
...
void Session::operator()(Trace<fix::codec::Reject> const &event, fix::Header const &) {
  ...
  switch (state_) {
    ...
    case WAITING_LOGON: {
      ...
      if (header.target_comp_id != shared_.settings.client.comp_id) {
        ...
        send_reject_and_close(header, fix::SessionRejectReason::OTHER, fix::codec::Error::UNKNOWN_TARGET_COMP_ID);
        return;  // note!
      }
      // validate: encrypt_method
      if (logon.encrypt_method != fix::EncryptMethod::NONE) {
        log::error(R"(Unexpected encrypt_method={} (expected: NONE))"sv, logon.encrypt_method);
        send_reject_and_close(header, fix::SessionRejectReason::OTHER, fix::codec::Error::INVALID_LOGON_ENCRYPT_METHOD);
        return;  // note!
      }
      ...
      if (heart_bt_int < shared_.settings.client.logon_heartbeat_min || heart_bt_int > shared_.settings.client.logon_heartbeat_max) {
        ...
        send_reject_and_close(header, fix::SessionRejectReason::OTHER, fix::codec::Error::INVALID_LOGON_HEART_BT_INT);
        return;  // note!
      }
      ...
      if (!logon.reset_seq_num_flag) {
        ...
        send_reject_and_close(header, fix::SessionRejectReason::OTHER, fix::codec::Error::INVALID_LOGON_RESET_SEQ_NUM_FLAG);
        return;  // note!
      }
      // authenticate
      auto success = [&](auto strategy_id) {
        ...
        try {
          ...
          auto user_request = fix::codec::UserRequest{
              ...
              .user_request_type = fix::UserRequestType::LOG_ON_USER,
              .username = party_id_,
              .password = {},
              .new_password = {},
          };
          ...
          send_reject_and_close(header, fix::SessionRejectReason::OTHER, fix::codec::Error::NOT_READY);
        }
      };
      auto failure = [&](auto &reason) {
        log::error("Invalid logon (reason: {})"sv, reason);
        send_reject_and_close(header, fix::SessionRejectReason::OTHER, fix::codec::Error::INVALID_LOGON);
      };
    ...
    case WAITING_CREATE_ROUTE:
    case READY:
      send_reject_and_close(header, fix::SessionRejectReason::OTHER, fix::codec::Error::UNEXPECTED_LOGON);
      break;
    case WAITING_REMOVE_ROUTE:
      make_zombie();
      break;
...
void Session::operator()(Trace<fix::codec::TradingSessionStatusRequest> const &event, fix::Header const &header) {
  ...
  // XXX TODO
  send_business_message_reject(
      header, trading_session_status_request.trad_ses_req_id, fix::BusinessRejectReason::UNSUPPORTED_MESSAGE_TYPE, fix::codec::Error::UNEXPECTED_MSG_TYPE);
}
  ...
  switch (state_) {
    ...
    case READY: {
      ...
      if (!validate_req_id(security_list_request.security_req_id)) {
        send_business_message_reject(header, security_list_request.security_req_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_REQ_ID);
        return;
      }
      handler_(event, session_id_);
      break;
  ...
  switch (state_) {
    ...
    case READY: {
      ...
      if (!validate_req_id(security_definition_request.security_req_id)) {
        send_business_message_reject(header, security_definition_request.security_req_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_REQ_ID);
        return;
      }
      handler_(event, session_id_);
      break;
  ...
  switch (state_) {
    ...
    case READY: {
      ...
      if (!validate_req_id(security_status_request.security_status_req_id)) {
        send_business_message_reject(
            header, security_status_request.security_status_req_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_REQ_ID);
        return;
      }
      handler_(event, session_id_);
      break;
  ...
  switch (state_) {
    ...
    case READY: {
      ...
      if (!validate_req_id(market_data_request.md_req_id)) {
        send_business_message_reject(header, market_data_request.md_req_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_MD_REQ_ID);
        return;
      }
      handler_(event, session_id_);
      break;
  ...
  switch (state_) {
    ...
    case READY: {
      ...
      if (!validate_req_id(order_status_request.ord_status_req_id)) {
        send_business_message_reject(header, order_status_request.ord_status_req_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_REQ_ID);
        return;
      }
      if (add_party_ids(event, [&](auto &event_2) { handler_(event_2, session_id_); })) {
      } else {
        auto &[trace_info, order_status_request] = event;
        send_business_message_reject(header, order_status_request.cl_ord_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::UNSUPPORTED_PARTY_IDS);
      }
      break;
    }
    case WAITING_REMOVE_ROUTE:
  ...
  switch (state_) {
    ...
    case READY: {
      ...
      if (!validate_req_id(order_mass_status_request.mass_status_req_id)) {
        send_business_message_reject(header, order_mass_status_request.mass_status_req_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_REQ_ID);
        return;
      }
      if (add_party_ids(event, [&](auto &event_2) { handler_(event_2, session_id_); })) {
      } else {
        send_business_message_reject(
            header, order_mass_status_request.mass_status_req_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::UNSUPPORTED_PARTY_IDS);
      }
      break;
    }
    case WAITING_REMOVE_ROUTE:
  ...
  switch (state_) {
    ...
    case READY: {
      ...
      if (!validate_req_id(new_order_single.cl_ord_id)) {
        send_business_message_reject(header, new_order_single.cl_ord_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_CL_ORD_ID);
        return;
      }
      if (add_party_ids(event, [&](auto &event_2) { handler_(event_2, session_id_); })) {
      } else {
        send_business_message_reject(header, new_order_single.cl_ord_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::UNSUPPORTED_PARTY_IDS);
      }
      break;
    }
    case WAITING_REMOVE_ROUTE:
  ...
  switch (state_) {
    ...
    case READY: {
      ...
      if (!validate_req_id(order_cancel_request.cl_ord_id)) {
        send_business_message_reject(header, order_cancel_request.cl_ord_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_CL_ORD_ID);
        return;
      }
      if (!validate_req_id(order_cancel_request.orig_cl_ord_id)) {
        send_business_message_reject(header, order_cancel_request.orig_cl_ord_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_ORIG_CL_ORD_ID);
        return;
      }
      if (add_party_ids(event, [&](auto &event_2) { handler_(event_2, session_id_); })) {
      } else {
        ...
        send_business_message_reject(header, order_cancel_request.cl_ord_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::UNSUPPORTED_PARTY_IDS);
      }
      break;
    }
    case WAITING_REMOVE_ROUTE:
  ...
  switch (state_) {
    ...
    case READY: {
      ...
      if (!validate_req_id(order_cancel_replace_request.cl_ord_id)) {
        send_business_message_reject(header, order_cancel_replace_request.cl_ord_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_CL_ORD_ID);
        return;
      }
      if (!validate_req_id(order_cancel_replace_request.orig_cl_ord_id)) {
        send_business_message_reject(
            header, order_cancel_replace_request.orig_cl_ord_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_ORIG_CL_ORD_ID);
        return;
      }
      if (add_party_ids(event, [&](auto &event_2) { handler_(event_2, session_id_); })) {
      } else {
        ...
        // XXX FIXME should be execution report
        send_business_message_reject(
            header, order_cancel_replace_request.cl_ord_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::UNSUPPORTED_PARTY_IDS);
      }
      break;
    }
    case WAITING_REMOVE_ROUTE:
  ...
  switch (state_) {
    ...
    case READY: {
      ...
      if (!validate_req_id(order_mass_cancel_request.cl_ord_id)) {
        send_business_message_reject(header, order_mass_cancel_request.cl_ord_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_CL_ORD_ID);
        return;
      }
      if (add_party_ids(event, [&](auto &event_2) { handler_(event_2, session_id_); })) {
      } else {
        send_business_message_reject(header, order_mass_cancel_request.cl_ord_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::UNSUPPORTED_MSG_TYPE);
      }
      break;
    }
    case WAITING_REMOVE_ROUTE:
  ...
  switch (state_) {
    ...
    case READY: {
      ...
      if (!validate_req_id(request_for_positions.pos_req_id)) {
        send_business_message_reject(header, request_for_positions.pos_req_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_REQ_ID);
        return;
      }
      if (add_party_ids(event, [&](auto &event_2) { handler_(event_2, session_id_); })) {
      } else {
        auto &[trace_info, request_for_positions] = event;
        send_business_message_reject(header, request_for_positions.pos_req_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::UNSUPPORTED_PARTY_IDS);
      }
      break;
    }
    case WAITING_REMOVE_ROUTE:
  ...
  switch (state_) {
    ...
    case READY: {
      ...
      if (!validate_req_id(trade_capture_report_request.trade_request_id)) {
        send_business_message_reject(
            header, trade_capture_report_request.trade_request_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::INVALID_REQ_ID);
        return;
      }
      if (add_party_ids(event, [&](auto &event_2) { handler_(event_2, session_id_); })) {
      } else {
        auto &[trace_info, trade_capture_report_request] = event;
        send_business_message_reject(header, trade_capture_report_request.cl_ord_id, fix::BusinessRejectReason::OTHER, fix::codec::Error::UNSUPPORTED_MSG_TYPE);
      }
      break;
    }
    case WAITING_REMOVE_ROUTE:
...
void Session::operator()(Trace<fix::codec::MassQuote> const &, fix::Header const &) {
  log::fatal("NOT IMPLEMENTED"sv);
}

void Session::operator()(Trace<fix::codec::QuoteCancel> const &, fix::Header const &) {
  log::fatal("NOT IMPLEMENTED"sv);
}

// helpers

void Session::send_reject_and_close(fix::Header const &header, fix::SessionRejectReason session_reject_reason, fix::codec::Error error) {
...
void Session::send_business_message_reject(
    fix::Header const &header, std::string_view const &ref_id, fix::BusinessRejectReason business_reject_reason, fix::codec::Error error) {
...
bool Session::add_party_ids(Trace<T> const &event, Callback callback) const {
  ...
  if (std::empty(value.no_party_ids)) {
    auto party = fix::codec::Party{
        .party_id = party_id_,
        .party_id_source = fix::PartyIDSource::PROPRIETARY_CUSTOM_CODE,
        .party_role = fix::PartyRole::CLIENT_ID,
    };
    auto value_2 = value;
